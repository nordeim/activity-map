...
- **E2E (Playwright, 54 checks)**: boots the PRODUCTION standalone server on :3100 against its own `db/e2e.db` (schema-pushed + seeded by the global setup). Suites: `auth.spec.ts` (logged-out surface + login flow + the session-10 shadcn login chrome + the session-12 white body), `browse.spec.ts` (unified browse planner + auto-forward params + the session-12 compact 38px/12px chips, browse cards, detail booking-request round-trip + rating pill/photo overlays + the session-10 max-w-6xl rounded-36 card, favourites round-trip + the session-12 grid overlay + 14px #3A3A3A subtitle, map pills/stats + the session-12 full-width search + 41px pills, profile chrome + the session-12 two-card layout with the 72px NAME h1), `home.spec.ts` (white planner card, the session-10 hero geometry, desktop floating-pill navbar, the session-10 category-card internals + mobile snap carousel + the session-12 compacted height, text route cards + early pin swap, blue restaurants carousel + six-card mobile deck, letter-reveal spans + the session-12 full-width left-aligned heading, the session-12 stay-showcase ORDER, 24px mobile titles, dark More-Things pill, square stay/sight cards, footer, home-place detail resolution, browse purity), `browse.spec.ts` footer-on-every-page checks, `mobile-navigation.spec.ts` (the five v4 failure classes + tap navigation at 390 / 640 / 1280).
