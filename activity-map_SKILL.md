...
project_state: "42 unit + 27 smoke + 56 E2E green; 42 published + 27 home-only + 9 map-demo places; session-16 profile/map parity on top of session-14 (the deployed mirror re-audited via browser E2E — all functional flows green incl. favourites + booking round-trips — and the live source re-measured at 1280/390: the profile page is now the live's CHROME-LESS surface in the (bare) route group — no Navbar, no footer, a full-page fixed 18px grid overlay at 40% opacity, the px-5/pt-10 -> md:px-8/md:pt-16 outer with the max-w-4xl main (h1 y~203/167), the centered-mobile identity with sun/heart chip icons and the white/80 Go back/Sign out pills; the map list cards are the live's FOUR-row layout — eyebrow + price on one justified row, 15px/600 title, 12px #888580 neighborhood, do-places eyebrow = sub-category uppercase (LANTERN WALK / ROOFTOP MUSIC / ART WORKSHOP) — in the live's interleaved order (Brass & Marble first, map.json reordered); the mobile home planner card is 358px at x=16 with the 4px grid gap (-mx-2 + w-[calc(100%+16px)]); the desktop category cards carry 46px rows + 34x35 icon cells with the near-black 229x54 View All hanging below the glass, the mobile carousel keeps the 36px/28x28 internals at radius 24; prior session-14 surfaces retained: column-major stay grid 381px, browse/map headings px-5/pt-16 -> md:px-8/md:pt-24, the hairline booking form with the 56/44 split, the scoped favourites overlay, the px-6 hero, the 1120px sights grid; observed live-site bug: the hosted app's SavedPlace POST 403s)"
---
