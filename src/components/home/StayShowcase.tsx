...
// (sessions 6 + 8): the big serif headline ("Choose Your Vibe, Select The
// Dates & Enjoy Your Ultimate Getaway", #1A1A1A) with the live's per-letter
// scroll reveal (cream → ink, LetterReveal), a one-line mood sub, and all
// twelve stays as SQUARE photo cards (the shared StayCard design: aspect 1/1,
// rounded 24, dark bg, white Inter 18px dsk / 24px mob title overlaid at the
// photo bottom, address + "€€€ · ★ rating" meta, ghost Learn More + white
// Book Now pills, heart overlay) in a full-bleed 3-column grid from md (the
// live's ~1144px inner grid) — one column on phones.

export function StayShowcase({ stays }: { stays: PlaceDTO[] }) {
  if (stays.length === 0) return null;
  return (
    <section id="stay-showcase" className="w-full py-16 sm:py-24">
      <div className="mx-auto mb-12 max-w-3xl px-4 text-center sm:px-6">
        <LetterReveal
          ...
          className="font-serif text-[40px] leading-[1.08] tracking-[-0.06em] sm:text-[clamp(40px,7.2vw,112px)]"
        />
