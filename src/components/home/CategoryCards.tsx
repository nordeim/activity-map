...
// re-measured from the live app (session 3): GLASS cards (bg white/34,
// radius 20px, the planner pill's shadow with the inset highlight), Inter
// 14px/600 headings ("12 Hotels" …), three curated rows with 13px icons,
// and the BLACK #141413 View All pill (36px, 12px/600, ls 0.03em) — the
// violet button is gone. The cards tilt in 3D on hover (the live app's
// rotateY perspective on the outer cards).
    ...
    <section aria-label="Browse the guide" className="relative z-10 -mt-10 sm:-mt-16">
      <div className="grid grid-cols-1 gap-5 md:grid-cols-3 md:gap-6 [perspective:1200px]">
        {CARDS.map(({ category, href, countLabel, rows, perspective }) => (
          <article
            key={category}
            className="group flex flex-col rounded-[20px] border border-white/40 bg-white/35 p-5 shadow-[0_8px_22px_rgba(0,0,0,0.12),inset_0_1px_0_rgba(255,255,255,0.42)] backdrop-blur-[28px] backdrop-saturate-150 transition-transform duration-500 ease-out sm:p-6"
          ...
          >
            ...
            <Link
              href={href}
              className="mx-auto mt-5 inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-full bg-[#141413] px-6 text-xs font-semibold tracking-[0.03em] text-white transition-colors duration-200 hover:bg-black"
            >
              View All
