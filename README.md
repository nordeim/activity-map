...
| Feature | Description |
|---------|-------------|
| 🧭 **Home, re-measured + remediated (sessions 3 + 6)** | Traveller-photo hero with the huge Libre Baskerville wordmark (full-bleed container — no clipping), the white elevated PLANNER CARD below md (radius 30, gray field pills, "Let's Plan Your Trip"; the frosted glass pill from md), near-opaque category cards with black `#141413` VIEW ALL pills on desktop and violet `#571AFF` full-width pills on mobile |
| 🗓 **Recommended Route** | The scroll-driven itinerary — 140vh heading trap, sticky route visual with a scroll-synced progress pill ("0% of your day planned"); below lg the five PHOTO stop cards (time pill + serif title on the cover photo + white detail panel + inline meta + black Learn More) flow down a dashed timeline, from lg a 340vh trap pins the visual and swaps ONE card at a time |
| 💙 **Highlighted Restaurants** | The vivid blue band (`#4D61FF`): desktop = a scroll-driven carousel (restaurant-name watermark, tilted floating photos, one frosted-glass detail card with Book a Table / Learn More); mobile = a sticky-stacked 16-card deck (browse-style cards sliding over each other) |
| 🛏 **Choose Your Vibe** | The full twelve-stay showcase as SQUARE photo cards (aspect 1/1, radius 24, white Inter 18px titles overlaid, "€€€ · ★ rating" meta, ghost Learn More + white Book Now pills) in a 3-column grid from md |
| 🏛 **Highlighted Sights** | Six calm stops (Fuggerei → Schaezlerpalais) as square photo cards with white overlaid titles, hanging meta panels, heart overlays, and the More Things to Do hand-off |
...
| 👤 **Profile** | PROFILE eyebrow, the USERNAME as the h1 (session-6 re-measure), "Your Roam account", "Augsburg", 0-day-streak + Explorer badge, Saved-places count, TRIPS/My bookings, Upcoming/Past/All tabs + Eat/Stay/Do filters, and the "No upcoming reservations. Time to explore." empty state |
...
| 🔐 **Cookie-session auth** | scrypt password hashing + HMAC-SHA256-signed stateless cookies, login rate limiting (10/IP/15 min), auth-gated route group with server-side redirects, and the live-parity login card (Welcome to Activity Map + Google/or/forgot-password chrome) |
...
| E2E | `bun run build && bun run test:e2e` | 45 | Playwright drives the production standalone server on :3100 with its own seeded DB |
