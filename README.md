...
| 🗓 **Recommended Route** | The scroll-driven itinerary — 140vh heading trap, sticky route visual with a scroll-synced progress pill ("0% of your day planned"); below lg (session-18): a FULL-VIEWPORT route visual (the winding-path svg with five numbered waypoints) pins sticky for ~208vh BEFORE the five TEXT stop cards flow (session-20: the panel pads `28px 18px 48px` with 28px card gaps; white time pill px-3/py-1 12px/400 NO shadow + dark serif 44px title + white rounded-28 max-w-md info card with the 20px/600 h3 place title + 13px #72706A inline meta + 13px/600 black Learn More) — no mobile progress chip, no dashed timeline (the live dropped both); from lg (session-20) a 50/50 split — the visual panel pins LEFT at half the viewport (640px at 1280) while the stop cards ride the right half (576px wide inside px-8, the card SLOT at y≈237) translating upward CONTINUOUSLY with scroll: card i crosses the slot at progress i/(N−1), every card moves at 0.665px per scroll px, tent-fading ±380px around the slot with 120ms linear transitions — exiting cards rise OUT, and the crossfade window shows two adjacent cards at once (the live's scroll-linked choreography) |
| 💙 **Highlighted Restaurants** | The vivid blue band (`#4D61FF`): desktop = a scroll-driven carousel over a 460vh trap (restaurant-name watermark, tilted floating photos, one frosted-glass detail card with Book a Table / Learn More) — session-18: the band now RISES over the route trap's tail (`lg:-mt-[800px]`, its top starting at the route sticky's release point exactly like the live); mobile = a FLOWING list (session-18: the live dropped the sticky-stacking deck) of the FIRST SIX restaurants as static cards spaced ~130px apart (~620px advances) |
