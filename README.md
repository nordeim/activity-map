...
| 👤 **Profile** | PROFILE eyebrow, the USERNAME as the h1 (session-6 re-measure), the email directly under the name (session-8), "Augsburg", 0-day-streak + Explorer badges, the dark Saved-places button into /favourites, a Back-to-home control, TRIPS/My bookings, Upcoming/Past/All tabs + Eat/Stay/Do filters WITH icons, and the "No upcoming reservations. Time to explore." empty state |
...
| E2E | `bun run build && bun run test:e2e` | 52 | Playwright drives the production standalone server on :3100 with its own seeded DB |
