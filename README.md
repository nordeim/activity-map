...
| 🛏 **Choose Your Vibe** | The full twelve-stay showcase (session-12 order: the live's shuffled home sequence — Courtyard Stay first, browse order untouched) as SQUARE photo cards (aspect 1/1, radius 24, white Inter 18px dsk / 24px mob titles overlaid, "€€€ · ★ rating" meta, ghost Learn More + white Book Now pills) in a 3-column grid from md (session-12: the 1178px inner grid, wrapper pt-112/pb-144) under the big serif heading — session-12: FULL-WIDTH LEFT-ALIGNED at #1A1A1A with the live's per-letter scroll reveal (cream → ink, LetterReveal) and the 14px #8A8780 centered subtitle |
...
| ❤️ **Favourites** | One-tap save/unsave on cards and detail pages (the 44×44 dark-glass heart, session-10), with a dedicated Favourites view (session-12: the 18px graph-paper grid texture back at 40% opacity, 55px serif heading, the 14px #3A3A3A subtitle, the live's Inter empty state) and an illustrated empty state |
| 👤 **Profile** | Session-12 redesign into TWO glass cards (896px): the identity card (rounded-36, bg-white/78, border-white/70) with the Profile eyebrow, the account NAME as the h1 ("Explorer", 72px serif), "Your Roam account" 16px #555550, the cream outlined Augsburg / 0-day-streak / Explorer chips, and the dark heart Saved-places button into /favourites; the bookings card (rounded-32, mt-8) with the 36px "My bookings" h2, FULL-WIDTH 44px/12px Upcoming/Past tabs, 38px All/Eat/Stay/Do filters, and the white rounded-26 empty state; a Back-to-home control + Sign out top row |
