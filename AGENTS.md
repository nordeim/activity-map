...
| Seed the guide (42 places + 27 home-only rows + demo user) | `bun run db:seed` |
| End-to-end smoke suite (27 checks; needs a build) | `./scripts/smoke-test.sh` |

**Gate order before every push:** `bun run lint` → `bun run typecheck` → `bun run test` (32) → `bun run build` → `./scripts/smoke-test.sh` (27, all must pass) → `bun run test:e2e` (35 Playwright checks — boots the standalone server on :3100 against its own `db/e2e.db`). There is no hosted CI; the local gate is the only gate.
...
- **Schema changes use `db push`, not migrations** (`prisma/migrations/` does not exist). `bun run db:seed` is idempotent — it wipes and reseeds domain tables from `prisma/data/{eat,stay,do,home}.json`: the 12+12+18 places captured from the live app PLUS 27 **home-only showcase rows** (`status: "home"` — 5 route stops, 6 sights, 16 restaurants) with deterministic per-neighborhood coordinates.
- **Home-only places are invisible to browses and counts by design.** `listPlacesForUser`/`countPlaces` filter `status: "published"` (so Eat/Stay/Do stay 12/12/18 and the category cards stay correct), while `getPlaceBySlug` does NOT filter status — `/place/home-route-*`, `/place/home-sight-*`, `/place/home-restaurant-*` resolve like any place. `listHomePlaces(prefix, userId)` in `src/lib/places.ts` selects them by slug prefix. The home page composition (hero → category cards → Recommended Route → blue Highlighted Restaurants → Choose Your Vibe stay showcase → Highlighted Sights → footer) mirrors the live app measured session 2.
- **Server Components by default; `"use client"` only where interactivity demands it** (the 12 client components: LoginForm, Navbar, Hero, HighlightedRestaurants, CategoryExplorer, PlaceCard, SaveButton, BookingForm, MapExplorer, LeafletCanvas, FavouritesView, ProfileView). Category pages and the home showcase sections (RecommendedRoute, StayShowcase, HighlightedSights, SiteFooter, CategoryCards) are server components that query Prisma directly.
...
- **Tailwind v4 is CSS-first: there is NO `tailwind.config.*`.** Design tokens live in `src/app/globals.css` under `@theme` (`--color-cream/-ink/-roam/-electric`, `--font-sans` (Inter) / `--font-serif` (Libre Baskerville — the live app's display serif) / `--font-nav` (Poppins — the live app's nav font), shadows, `--radius-4xl`) plus `@utility` primitives (`bg-grid`, `no-scrollbar`, `hero-shade`). Extend the theme there, not in a config file. The known v4 mobile-nav failure classes (no-nav / invisible / clipped / under-layer / breakpoint mismatch) are regression-pinned by `tests/e2e/mobile-navigation.spec.ts`; the Navbar keeps the `no-scrollbar` horizontal overflow as a safety valve — don't remove it. The nav wordmark is the live app's image logo (`public/images/roam-logo.png`) rendered with the reference's two-span crop technique — the mobile wordmark span must stay ≤52px wide or the 390px bar overflows (failure class D).
- **Mobile chrome differs from desktop by design** (measured from the reference): below `sm` the navbar is a full-width flat top bar with TEXT-ONLY links (icons hidden), the image wordmark, and 12px link gaps; Map folded into the right-cluster pin; from `sm` up it becomes the floating white pill with icons + avatar chip. Don't "fix" the asymmetry.
